...
class Battle():
    ...
    def is_in_range(self,coord_army1,coord_army2,attacker,victim):
        ...
        if (in_range_by_x && in_range_by_y):
            return True
        return False
    ...
    def bot_1step(self, army1, army2,coord_army1,coord_army2, attacker):
        attacker=1
        alive_list=get_alive(army2)
        ...
        while(!victim_chosen):
            for i in range('остаток хода'):
                ...
                if (no_in_range+distanse_x+distance_y<self.units_list[attacker].get_abil('move',0)):#если дотопает за ход
                    ...
                    in_range_by_x=distance_x<=0
                    in_range_by_y=distance_y<=0
                    if (in_range_by_x && in_range_by_y):#если кто-то уже в радиусе атаки
    ...
    def who_wins(self):
        ...
        if(countwarrs1>0 && countwarrs2>0):
            return 0
        elif(countwarrs1==0):
            return 1
        else:
    ...
    def auto_battle(self,army1,army2, coord_army1, coord_army2):
...
bat.kick([0,10,0,0,0,0],[0,0,20,0,0,0],1,2)